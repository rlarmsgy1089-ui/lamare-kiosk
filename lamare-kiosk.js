...
function dismissModal(){var els=document.querySelectorAll('button,a,span,div');for(var i=0;i<els.length;i++){var e=els[i];if(!vis(e)||e.children.length)continue;if(n(e.textContent)==='확인'){ck(e);return true;}}return false;}
...
function calibrate(){
  ...
  bar.innerHTML='<div style="display:flex;align-items:center;gap:14px;"><span id="lk-step" style="flex:none;width:42px;height:42px;border-radius:50%;background:#fff;color:#2f3e6b;font-size:20px;font-weight:800;display:flex;align-items:center;justify-content:center;">1</span><span id="lk-msg" style="font-size:clamp(16px,3.4vw,21px);font-weight:600;">화면의 <b>숫자 키패드에서 아무 숫자</b>를 한 번 누르세요</span></div><button id="lk-cancel" style="flex:none;background:rgba(255,255,255,.2);border:none;color:#fff;padding:10px 18px;border-radius:10px;cursor:pointer;font-size:15px;font-weight:700;">취소</button>';
  ...
  function onClick(e){
    ...
    if(stage===1){var r=e.target,found=null;for(var k=0;k<8;k++){if(!r)break;if(padFromRoot(r)){found=r;break;}r=r.parentElement;}
      ...
      localStorage.setItem('lamare_kp',cssPath(found));flash(found);stage=2;stepb.textContent='2';msg.innerHTML="\u2705 좋아요! 이번엔 <b>\u20182시간 무료\u2019 할인권</b>을 한 번 누르세요";
    }else{localStorage.setItem('lamare_cp',cssPath(e.target));flash(e.target);msg.innerHTML='\u2705 완료되었습니다!';setTimeout(function(){end(true);},600);}
  }
